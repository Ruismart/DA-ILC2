## ref to https://github.com/PeeperLab/Rtoolbox/blob/master/R/ReplotGSEA.R
...
replotGSEA <- function(path, gene.set, class.name, metric.range,
  ...
  if(gsea.enrichment.score < 0) {
    text(length(gsea.rnk$metric) * 0.075, plot.coordinates[3] * 0.92,
         paste0("FDR: ", gsea.fdr))
  ...
  text(length(gsea.rnk$metric) * 0.02, 0.7, "DA", adj = c(0, NA))
  text(length(gsea.rnk$metric) * 0.98, 0.7, "PBS", adj = c(1, NA))
