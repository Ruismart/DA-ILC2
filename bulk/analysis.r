...
easy_OE <- function(mat_e,cells_s,path_n,gene_sign,seed_r=7788){
  ...
  ret$sig.names <- names(gene.sign)   # path_o
  ...
  for(i in ret$sig.names){
    ...
    # OE barplot    
    ret$bar[[i]] <- ggplot(data=cbind.data.frame(Score=ret$sig.scores[,i],
                                                 Name=factor(names(ret$sig.scores[,i]),levels=names(ret$sig.scores[,i]))),
                           mapping=aes(x=Score,y=Name)) +
                           geom_bar(stat='identity') +
                           #coord_flip() +
                           labs(y="",x=paste0("Overall Expression of pathway:\n",path_n[[i]]))
    ...
    ret$mat_z[[i]] <- ret$mat_z[[i]][names(idx_genes),rev(names(ret$sig.scores[,i]))]


    # mat_z heatmap
    ret$heat[[i]] <- pheatmap(t(ret$mat_z[[i]]), cluster_cols=FALSE,cluster_rows=FALSE,fontsiize_row=7.5,
                              main=paste0("Row Zscore of genes in pathway: ",path_n[[i]]),
